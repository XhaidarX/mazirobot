...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        async for i in ubot2.iter_messages(
            "@logomazi", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
        fpath_ = glob.glob("./Mazi/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Kazu Robot](https://t.me/Dark_userbot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./Mazi/resources/fonts"):
            os.remove(font_)
...
async def logo_(event):
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Kazu Robot](https://t.me/Dark_musikbot)",
            force_document=False,
        )
        os.remove(flnme)
        await xx.delete()
...
⩺ /wlogo <teks/nama> - Buat logo dengan tampilan lebar saja.
 Editor Gambar :
⩺  /edit <balas foto> -  untuk edit.
"""
